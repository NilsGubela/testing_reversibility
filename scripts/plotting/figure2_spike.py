...
data_spike = pd.read_csv(out_dir / "all_seq_spike.csv")
data_spike = data_spike.rename(columns={"stat_b": "Bowker", "stat_s": "Stuart", "stat_qs": "QS"})
data_spike = evaluate_tests(data_spike)
data_spike = add_decision(data_spike)

data_spike = data_spike.merge(
...
data_spike["t"] = data_spike["Collection_Date"]

data_spike["all_nonzero_flag"] = (
    data_spike[cols].ne(0).all(axis=1) & data_spike[cols].notna().all(axis=1)
).astype(int)
data_spike["real_final_decision"] = np.where(
    data_spike["all_nonzero_flag"].eq(1),
    data_spike["final_decision"],
    np.nan
)

data_spike["y"] = data_spike["real_final_decision"].map(y_map)


data_tmp = data_spike.dropna(subset=["t"])
data_tmp1 = data_tmp
data_tmp = data_tmp[data_tmp["y"] > 0]
with plt.rc_context(STYLE):
    fig = plt.figure(figsize=(double_col, 2*1.5))
    fig.suptitle("SARS-CoV-2 spike region", weight='bold', fontsize = 10)
    ...
    plt.savefig(fig_dir /"figure2_spike_raw.svg")
    plt.show()
